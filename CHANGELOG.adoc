...
* Allow merge results to push from agent nodes, not just from controller
...
* Handle special meaning of some characters on Windows (https://issues.jenkins-ci.org/browse/JENKINS-13007[JENKINS-13007])
...
* Fixed issue with polling failing if the controller has 0 executors.
...
tag reference (jbq)
* Fix https://issues.jenkins-ci.org/browse/JENKINS-2675[JENKINS-2675], Git fails on remote agents (jbq)
