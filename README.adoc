...
In single link:https://www.jenkins.io/doc/book/pipeline/[pipeline] which build one specified branch(e.g. from parameters `params.branchName`)
...
checkout resolveScm(
  source: gitSource(
    ...
    targets: [env.BRANCH_NAME, 'main'] // [params.branchName, 'main'] if you pass branch name from build parameters
)
----
In this example we try find branch with name `env.BRANCH_NAME`/`params.branchName` in `ws-cleanup-plugin` repository if it non exists fallback to `main` branch and then clone it.
