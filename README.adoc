...
Checkout from the git client plugin source repository using ssh protocol, ssh private credentials, and a non-default branch.
...
Checkout from the git client plugin source repository using https protocol, username/password credentials, and a non-default branch.
...
Checkout from the git client plugin source repository using https protocol and a non-default branch.
...
Checkout from the git client plugin source repository using https with no credentials, a default branch, and without tags.
...
Checkout from the workspace cleanup plugin source repository using https without credentials, a default branch, and with refspec defined for the repository.
...
Checkout from the workspace cleanup plugin source repository using https without credentials, a default branch, and with prune tags extension enabled.
This removes remote tracking branches from the local workspace if they no longer exist on the remote.
...
Checkout from the workspace cleanup plugin source repository using https without credentials, a default branch, and clean checkout extension with delete untracked nested repositories enabled.
Clean the workspace after every checkout by deleting all untracked files and directories, including those which are specified in the `.gitignore` file.
