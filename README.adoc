...
The git plugin supports both simple, standalone pipelines and more complex multibranch pipeline workflows.

* A single pipeline is a straightforward Jenkins job configured to check out a repository and execute a sequence of build steps. It typically involves a single SCM checkout, and all necessary information about the repository (such as branch, commit, or URL) is obtained during the checkout step.
* A multibranch pipeline project automates the management of multiple branches within a repository. Jenkins automatically discovers branches, creates individual jobs for each, and triggers builds based on branch-specific configurations. This setup allows for scalable, branch-specific workflows, and each branch has its own SCM checkout context.
...
checkout scmGit(
    ...
    userRemoteConfigs: [[refspec: '+refs/heads/master:refs/remotes/origin/master',
        url: 'https://github.com/jenkinsci/ws-cleanup-plugin.git']])
----
...
==== Using resolveScm in pipelines

When you have a pipeline that checks out other sibling repositories, you may want to check out the matching branch from that sibling repository (assuming it exists) but fall back to the main branch if there is no matching branch.

For it you can use `resolveScm`. [Documentation](https://www.jenkins.io/doc/pipeline/steps/workflow-multibranch/#resolvescm-resolves-an-scm-from-an-scm-source-and-a-list-of-candidate-target-branch-names)

You can pass `cloneOption` trait with shallow flag for speed-up retrieve target branch.
Just add in `traits` param these code:
[source,groovy]
----
cloneOption(cloneOption(depth: 1, shallow: true))
----

In link:https://www.jenkins.io/doc/pipeline/steps/workflow-multibranch/[Multi-branch pipeline] `resolveScm` used for build all or specified(e.g. wildcard rule) branches. You can use `env.BRANCH_NAME` for get current branch this build.
...
checkout resolveScm(
  source: gitSource(
    ...
    targets: [env.BRANCH_NAME, 'main']
)
----
In this example we try find branch with name `env.BRANCH_NAME` if non exists just fallback to `main` branch and then clone it.

In single link:https://www.jenkins.io/doc/book/pipeline/[pipeline] which build one specified branch(e.g. from parameters)
...
checkout resolveScm(
  source: gitSource(
    ...
    traits: [cloneOption(cloneOption(depth: 1, shallow: true)), gitBranchDiscovery()]
    remote: 'https://github.com/jenkinsci/ws-cleanup-plugin'),
    targets: [params.branchName, 'main']
)
----
In this example we try find branch with name `params.branchName` if non exists just fallback to `main` branch and then clone it.
