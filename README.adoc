...
==== Resolve an SCM from an Git Source and a list of candidate target branch names before checkout

Find first matching branch from repository and checkout on this. link:https://www.jenkins.io/doc/pipeline/steps/workflow-multibranch/#resolvescm-resolves-an-scm-from-an-scm-source-and-a-list-of-candidate-target-branch-names[resolveScm] can be used for resolve dependency branch(e.g. the same name `task-1234`).
...
checkout resolveScm(
  source: gitSource(
    ...
    targets: [env.BRANCH_NAME, 'main', 'master']
)
----
...
checkout resolveScm(
  source: gitSource(
    ...
    traits: [cloneOption(cloneOption(depth: 1, shallow: true)), gitBranchDiscovery()]
    remote: 'https://github.com/jenkinsci/ws-cleanup-plugin'),
    targets: [params.branchName, 'main', 'master']
)
----
