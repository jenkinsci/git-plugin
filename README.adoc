...
Checkout from the git client plugin source repository using https protocol, ssh private credentials, and a non-default branch. The git plugin supports private key credentials provided by the link:https://plugins.jenkins.io/credentials[Jenkins credentials plugin].

[source,groovy]
---
checkout([$class: 'GitSCM',
          branches: [[name: 'disposer']],
          userRemoteConfigs: [[credentialsId: '9de6b979-0777-4e6c-88ad-bf86a9ac4098', url: 'https://github.com/jenkinsci/ws-cleanup-plugin']]])
---

==== Checkout with https and a username/password credential

Checkout from the git client plugin source repository using https protocol, username / password credentials, and a non-default branch. The git plugin supports username / password credentials provided by the link:https://plugins.jenkins.io/credentials[Jenkins credentials plugin].

[source,groovy]
---
checkout([$class: 'GitSCM',
          branches: [[name: 'jenkisnfile']],
          userRemoteConfigs: [[credentialsId: '9de6b979-0777-4e6c-88ad-bf86a9ac47e3', url: 'https://github.com/jenkinsci/ws-cleanup-plugin']]])
---

==== Checkout with git large file support enabled

Checkout from the git client plugin source repository using https no credentials, a non-default branch, and git large file support extension. link:https://git-lfs.github.com[Enable git large file support] for the workspace by pulling large files after the checkout completes.
...
Checkout from the git client plugin source repository using https no credentials, a default branch, and without tags saving time and disk space when you want to access only what is specified by the refspec, without considering any repository tags.
...
Checkout from the git client plugin source repository using https no credentials, a default branch, and with a shallow clone which  requests a limited number of commits from the tip of the requested branch(es) thereby saving time, data transfer and disk space.
...
==== Checkout with a narrow refspec (needs more reading)

Checkout from the git client plugin source repository using https no credentials, a default branch, and with refspec defined for the repository. This can save time, data transfer and disk space when you only need to access the references specified by the refspec.
...
Checkout from the git client plugin source repository using https no credentials, a default branch, and with prune tags extension enabled. This removes remote tracking branches from the local workspace if they no longer exist on the remote.
...
==== Clean after checkout and delete untracked nested repositories

Checkout from the git client plugin source repository using https no credentials, a default branch, and clean checkout extension with delete untracked nested repositories enabled. Clean the workspace after every checkout by deleting all untracked files and directories, including those which are specified in .gitignore.
...
==== Clean before checkout and delete untracked nested repositories

Checkout from the git client plugin source repository using https no credentials, a default branch, and clean checkout extension with delete untracked nested repositories enabled. Clean the workspace before every checkout by deleting all untracked files and directories, including those which are specified in .gitignore.
...
Checkout from the git client plugin source repository using https no credentials, a default branch, and clean wipe workspace extension which helps to delete the contents of the workspace before building, ensuring a fully fresh workspace.
