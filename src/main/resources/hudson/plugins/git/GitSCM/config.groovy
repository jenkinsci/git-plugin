...
f.entry(title:_("Repositories"), help:"/plugin/git/help.html") {
  f.repeatable(var:"repo", name:"repo", varStatus:"repoStatus", items:instance.repositories, minimum:"1", noAddButton:"false") {
    table(width:"100%") {
      f.entry(title:_("URL of repository"), field:"url") {
        f.textbox(value:repo.URIs.get(0).toPrivateString())
      }
      f.advanced() {
        ...
        f.entry(title:_("Refspec (blank to create default)"), field:"refspec") {
          f.textbox(value:repo.fetchRefSpecs.get(0))
        }
      }
      f.entry() {
        div(align:"right") {
...
f.advanced() {
  ...
  f.dropdownList(name:"buildChooser", title:_("Choosing strategy"), help:"/plugin/git/choosingStrategy.html") {
    current = instance.buildChooser
    descriptor.buildChooserDescriptors.each { cDesc ->
      shouldSelect = "false"
      if ((current.descriptor == cDesc) or
          ((current == null) and (cDesc.displayName=='Default'))) {
        shouldSelect = "true"
      }
  ...
  f.entry(title:_("Git executable"), field:"gitTool") {
    select(name:"gitTool") {
      descriptor.getTools.each { gitTool ->
        if (instance.gitTool == gitTool.name) {
