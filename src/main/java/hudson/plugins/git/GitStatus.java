...
public class GitStatus extends AbstractModelObject implements UnprotectedRootAction {
    ...
    public HttpResponse doNotifyCommit(@QueryParameter(required = true) String url, @QueryParameter(required = false) String branches) throws ServletException, IOException {
        ...
        try {
            ...
            for (AbstractProject<?, ?> project : Hudson.getInstance().getAllItems(AbstractProject.class)) {
                Collection<SCM> projectScms = getProjectScms(project);

                for (SCM projectScm : projectScms) {
                    GitSCM git = (GitSCM) projectScm;
                        ...
                        if (branchesArray.length == 1 && branchesArray[0] == "") {
                            ...
                            for (BranchSpec branchSpec : git.getBranches()) {
                                for (int i = 0; i < branchesArray.length; i++) {
                                    if (branchSpec.matches(repository.getName() + "/" + branchesArray[i])) {
                                        branchMatches = true;
                                        break;
                                    }
                                }
            ...
            final String msg;
            if (!scmFound) {
                msg = "No git jobs found";
            } else if (!urlFound) {
                msg = "No git jobs using repository: " + url + " and branches: " + branches;
            } else if (!triggerFound) {
                msg = "Jobs found but they aren't configured for polling";
            } else {
                msg = null;
            }
    ...
    private Collection<SCM> getProjectScms(AbstractProject<?, ?> project) {
        Set<SCM> projectScms = Sets.newHashSet();
