...
public class GitPublisher extends Recorder implements Serializable, MatrixAggregatable {
                    ...
                    mergeResult = workingDirectory.act(new FileCallable<Boolean>() {
                            ...
                            public Boolean invoke(File workspace,
	                            ...
	                            if (false == gitSCM.getSkipTag()) {
	                                ...
	                                RegisterTag(GetValidRemoteBranchName(gitSCM, remote), build, buildnumber, listener);
	                            }
                        ...
                        try {
                            tagResult = workingDirectory.act(new FileCallable<Boolean>() {
                                    ...
                                    public Boolean invoke(File workspace,
                                        ...
                                        String localTagMessage = tagMessage;
                                        if (localTagMessage.isEmpty()) {
                                            localTagMessage = "Jenkins Git plugin tagging with " + tagName;
                                            }

                                        git.tag(tagName, localTagMessage);

                                        RegisterTag(GetValidRemoteBranchName(gitSCM, remote), build, tagName, listener);
                                    } else if (!tagExists) {
    ...
    /**
     * Gets the remote branch name in the form of origin/branchName If more than
     * one branch is present the first (Head) branch is used to indicate a
     * taging
     */
    private String GetValidRemoteBranchName(final GitSCM gitSCM, RemoteConfig remote) {
        List<BranchSpec> branches = gitSCM.getBranches();
        String remoteBranchName = "";
        if (branches.size() > 0) {
            remoteBranchName = branches.get(0).getName();
        }
        String remoteBranchURL = (null == remote) ? "" : remote.getName(); //In the form of origin/branchName
        if (!remoteBranchURL.isEmpty() && !remoteBranchName.isEmpty()) {
            remoteBranchURL += "/";
        }
        remoteBranchURL += remoteBranchName;
        return remoteBranchURL;
    }

    /**
     * Get the existing tag action and register this tag this will allow us to
     * see all the tags applied from GitPublisher from the build page
     *
     */
    private void RegisterTag(String branchName, final AbstractBuild<?, ?> build, String tagName, final BuildListener listener) {
        listener.getLogger().println("Registering tag: " + tagName + ", for branch: " + branchName);
        GitTagAction tagAction = build.getAction(GitTagAction.class);
        BuildData bldData = build.getAction(BuildData.class);
        if (null == tagAction) {
            tagAction = new GitTagAction(build, bldData);
            build.addAction(tagAction);
        }
        tagAction.RegisterTag(branchName, tagName);
    }

    /**
     * Handles migration from earlier version - if we were pushing merges, we'll
     * be instantiated but tagsToPush will be null rather than empty.
     *
     * @return This.
     */
