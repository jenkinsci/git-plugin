 ...
 * @author Kohsuke Kawaguchi ... and many others
 */
public class GitSCM extends GitSCMBackwardCompatibility {
	...
	public GitSCM(String repositoryUrl) {
		this(createRepoList(repositoryUrl), Collections
				.singletonList(new BranchSpec("")), false, Collections
				.<SubmoduleConfig> emptyList(), null, null, null);
	}

	// @Restricted(NoExternalUse.class) // because this keeps changing
	@DataBoundConstructor
	public GitSCM(List<UserRemoteConfig> userRemoteConfigs,
		...
		this.extensions = new DescribableList<GitSCMExtension, GitSCMExtensionDescriptor>(
	...
	private void updateFromUserData() throws GitException {
		...
		try {
			...
			this.remoteRepositories = DescriptorImpl
					.createRepositoryConfigurations(pUrls, repoNames, refSpecs);
	...
	public Object readResolve() throws IOException {
		...
		if (source != null) {
			...
			remoteRepositories.add(newRemoteConfig("origin", source,
					rs.toArray(new RefSpec[0])));
			if (branch != null) {
		...
		if (remoteRepositories != null && userRemoteConfigs == null) {
			...
			for (RemoteConfig cfg : remoteRepositories) {
				...
				String refspec = "";
				if (cfg.getFetchRefSpecs().size() > 0
						&& cfg.getFetchRefSpecs().get(0) != null)
					refspec = cfg.getFetchRefSpecs().get(0).toString();

				userRemoteConfigs.add(new UserRemoteConfig(url, cfg.getName(),
						refspec, null));
			}
		}
		...
		if (extensions == null)
			extensions = new DescribableList<GitSCMExtension, GitSCMExtensionDescriptor>(
					Saveable.NOOP);

		readBackExtensionsFromLegacy();

		if (choosingStrategy != null
				&& getBuildChooser().getClass() == DefaultBuildChooser.class) {
				...
				if (choosingStrategy.equals(d.getLegacyId())) {
					try {
						...
						LOGGER.log(Level.WARNING,
								"Failed to instantiate the build chooser", e);
	...
	public RepositoryBrowser<?> guessBrowser() {
		if (remoteRepositories != null && remoteRepositories.size() == 1) {
			...
			if (uris.size() == 1) {
				...
				// TODO make extensible by introducing an abstract
				// GitRepositoryBrowserDescriptor
				...
				m = Pattern.compile("git@github[.]com:([^/]+/[^/]+)[.]git")
						.matcher(uri);
				if (m.matches()) {
	...
	public boolean isCreateAccountBasedOnEmail() {
		...
		return (gitDescriptor != null && gitDescriptor
	...
	public BuildChooser getBuildChooser() {
		...
		BuildChooserSetting bcs = getExtensions()
				.get(BuildChooserSetting.class);
		if (bcs != null)
	...
	@Deprecated
	public String getParamLocalBranch(Run<?, ?> build) throws IOException,
			InterruptedException {
		return getParamLocalBranch(build, new LogTaskListener(LOGGER,
				Level.INFO));
	}

	/**
	 * Gets the parameter-expanded effective value in the context of the current
	 * build.
	 */
	public String getParamLocalBranch(Run<?, ?> build, TaskListener listener)
			throws IOException, InterruptedException {
		String branch = getLocalBranch();
		// substitute build parameters if available
		return getParameterString(branch != null ? branch : null,
				build.getEnvironment(listener));
	}

	@Deprecated
	public List<RemoteConfig> getParamExpandedRepos(Run<?, ?> build)
			throws IOException, InterruptedException {
		return getParamExpandedRepos(build, new LogTaskListener(LOGGER,
				Level.INFO));
	}

	/**
	 * Expand parameters in {@link #remoteRepositories} with the parameter
	 * values provided in the given build and return them.
	 * 
	...
	/**
	 * Expand Parameters in the supplied remote repository with the parameter
	 * values provided in the given environment variables }
	 * 
	 * @param env
	 *            Environment variables with parameter values
	 * @param remoteRepository
	 *            Remote repository with parameters
	 * @return remote repository with expanded parameters
	 */
	public RemoteConfig getParamExpandedRepo(EnvVars env,
			RemoteConfig remoteRepository) {
		return newRemoteConfig(
				getParameterString(remoteRepository.getName(), env),
				getParameterString(remoteRepository.getURIs().get(0)
						.toPrivateString(), env),
	...
	public List<RemoteConfig> getRepositories() {
		// Handle null-value to ensure backwards-compatibility, ie project
		// configuration missing the <repositories/> XML element
		if (remoteRepositories == null) {
	...
	private List<RefSpec> getRefSpecs(RemoteConfig repo, EnvVars env) {
		...
		for (RefSpec refSpec : repo.getFetchRefSpecs()) {
			refSpecs.add(new RefSpec(
					getParameterString(refSpec.toString(), env)));
		}
		return refSpecs;
	}

	/**
	 * If the configuration is such that we are tracking just one branch of one
	 * repository return that branch specifier (in the form of something like
	 * "origin/master" or a SHA1-hash
	 * 
	...
	@Override
	public SCMRevisionState calcRevisionsFromBuild(Run<?, ?> abstractBuild,
			FilePath workspace, Launcher launcher, TaskListener taskListener)
			throws IOException, InterruptedException {
	...
	public boolean requiresWorkspaceForPolling() {
		...
		// TODO would need to use
		// hudson.plugins.git.util.GitUtils.getPollEnvironment
		return getSingleBranch(new EnvVars()) == null;
	}

	@Override
	public PollingResult compareRemoteRevisionWith(Job<?, ?> project,
			Launcher launcher, FilePath workspace, final TaskListener listener,
			SCMRevisionState baseline) throws IOException, InterruptedException {
		try {
			return compareRemoteRevisionWithImpl(project, launcher, workspace,
					listener);
		} catch (GitException e) {
	...
	private static Node workspaceToNode(FilePath workspace) { // TODO
																// https://trello.com/c/doFFMdUm/46-filepath-getcomputer
	...
	private PollingResult compareRemoteRevisionWithImpl(Job<?, ?> project,
			Launcher launcher, FilePath workspace, final TaskListener listener)
			throws IOException, InterruptedException {
		// Poll for changes. Are there any unbuilt revisions that Hudson ought
		// to build ?

		listener.getLogger().println(
				"Using strategy: " + getBuildChooser().getDisplayName());
		...
		if (singleBranch != null // branch spec can be resolved to a single
									// branch
				&& buildData.lastBuild != null
				&& buildData.lastBuild.getMarked() != null // we know previous
															// build commit
				&& !requiresWorkspaceForPolling() // remote polling hasn't been
													// intentionally disabled
		) {

			final EnvVars environment = project instanceof AbstractProject ? GitUtils
					.getPollEnvironment((AbstractProject) project, workspace,
							launcher, listener, false) : new EnvVars();

			GitClient git = createClient(listener, environment, project,
					Jenkins.getInstance(), null);

			String gitRepo = getParamExpandedRepos(lastBuild, listener).get(0)
					.getURIs().get(0).toString();
			ObjectId head = git.getHeadRev(gitRepo, singleBranch);
			if (head != null) {
				listener.getLogger().println(
						"[poll] Latest remote head revision is: "
								+ head.getName());
		...
		final EnvVars environment = project instanceof AbstractProject ? GitUtils
				.getPollEnvironment((AbstractProject) project, workspace,
						launcher, listener) : new EnvVars();

		FilePath workingDirectory = workingDirectory(project, workspace,
		...
		GitClient git = createClient(listener, environment, project,
				workspaceToNode(workspace), workingDirectory);
		...
		if (git.hasGitRepo()) {
			...
			// Fetch updates
			for (RemoteConfig remoteRepository : getParamExpandedRepos(
					lastBuild, listener)) {
			...
			Collection<Revision> candidates = getBuildChooser()
					.getCandidateRevisions(
							...
							listener,
							buildData,
							new BuildChooserContextImpl(project, null,
									environment));
	...
	/**
	 * Allows {@link Builder}s and {@link Publisher}s to access a configured
	 * {@link GitClient} object to perform additional git operations.
	 */
	public GitClient createClient(TaskListener listener, EnvVars environment,
			Run<?, ?> build, FilePath workspace) throws IOException,
			InterruptedException {
		FilePath ws = workingDirectory(build.getParent(), workspace,
				environment, listener);
		/* ws will be null if the node which ran the build is offline */
		if (ws != null) {
			ws.mkdirs(); // ensure it exists
		}
		return createClient(listener, environment, build.getParent(),
				workspaceToNode(workspace), ws);
	}

	/* package */GitClient createClient(TaskListener listener,
			EnvVars environment, Job project, Node n, FilePath ws)
			throws IOException, InterruptedException {
			...
			if (uc.getCredentialsId() != null) {
				...
				StandardUsernameCredentials credentials = CredentialsMatchers
						.firstOrNull(CredentialsProvider.lookupCredentials(
								...
								ACL.SYSTEM, URIRequirementBuilder.fromUri(url)
										.build()), CredentialsMatchers.allOf(
	...
	private BuildData fixNull(BuildData bd) {
		return bd != null ? bd : new BuildData(getScmName(),
				getUserRemoteConfigs()) /* dummy */;
	}
	 ...
	 * @throws IOException
	 */
	private void fetchFrom(GitClient git, TaskListener listener,
		...
		for (URIish url : remoteRepository.getURIs()) {
			try {
				if (first) {
					git.setRemoteUrl(remoteRepository.getName(),
							url.toPrivateASCIIString());
					first = false;
				} else {
					git.addRemoteUrl(remoteRepository.getName(),
							url.toPrivateASCIIString());
				}

				FetchCommand fetch = git.fetch_().from(url,
						remoteRepository.getFetchRefSpecs());
				...
				throw new GitException(
						"Failed to fetch from " + url.toString(), ex);
			}
		}
	}
	...
	public GitTool resolveGitTool(TaskListener listener) {
		...
		GitTool git = Jenkins.getInstance()
		...
		for (GitSCMExtension ext : extensions) {
			try {
				...
				throw new RuntimeException(
						ext.getDescriptor().getDisplayName()
								+ " extended Git behavior is incompatible with other behaviors");
			}
		}
	...
	public AbstractBuild<?, ?> getBySHA1(String sha1) {
		AbstractProject<?, ?> p = Stapler.getCurrentRequest()
	...
	/* package */static class BuildChooserContextImpl implements
			BuildChooserContext, Serializable {
		final Job project;
		final Run build;
		...
		private Object writeReplace() {
			return Channel.current().export(BuildChooserContext.class,
					new BuildChooserContext() {
						public <T> T actOnBuild(
								ContextCallable<Run<?, ?>, T> callable)
								throws IOException, InterruptedException {
							return callable.invoke(build, Channel.current());
						}

						public <T> T actOnProject(
								ContextCallable<Job<?, ?>, T> callable)
								throws IOException, InterruptedException {
	...
	/**
	 * Determines the commit to be built in this round, updating the working
	 * tree accordingly, and return the information about the selected commit.
	 * 
	 * <p>
	 * For robustness, this method shouldn't assume too much about the state of
	 * the working tree when this method is called. In a general case, a working
	 * tree is a left-over from the previous build, so it can be quite messed up
	 * (such as HEAD pointing to a random branch.) It is expected that this
	 * method brings it back to the predictable clean state by the time this
	 * method returns.
	 */
	private @NonNull
	Build determineRevisionToBuild(final Run build, final BuildData buildData,
			final EnvVars environment, final GitClient git,
		...
		if (build instanceof MatrixRun) {
			...
			if (parentBuild != null) {
				...
				if (parentBuildData != null) {
					...
					if (lastBuild != null)
						candidates = Collections.singleton(lastBuild
								.getMarked());
				}
			}
		}
		...
		if (candidates.isEmpty()) {
			final RevisionParameterAction rpa = build
					.getAction(RevisionParameterAction.class);
			if (rpa != null) {
		...
		if (candidates.isEmpty()) {
			final String singleBranch = environment
					.expand(getSingleBranch(environment));

			final BuildChooserContext context = new BuildChooserContextImpl(
					build.getParent(), build, environment);
			candidates = getBuildChooser().getCandidateRevisions(false,
					singleBranch, git, listener, buildData, context);
		}

		if (candidates.isEmpty()) {
			// getBuildCandidates should make the last item the last build, so a
			// re-build
		...
		for (GitSCMExtension ext : extensions) {
			rev = ext.decorateRevisionToBuild(this, build, git, listener,
					marked, rev);
		}
		...
		if (candidates.size() > 1) {
			...
			if (job instanceof AbstractProject) {
				...
				if (!project.isDisabled()) {
					log.println("Scheduling another build to catch up with "
							+ project.getFullDisplayName());
					if (!project
							.scheduleBuild(
									0,
									new SCMTrigger.SCMTriggerCause(
											"This build was triggered by build "
													+ build.getNumber()
	...
	/**
	 * Retrieve Git objects from the specified remotes by doing the likes of
	 * clone/fetch/pull/etc.
	 * 
	 * By the end of this method, remote refs are updated to include all the
	 * commits found in the remote servers.
	 */
	private void retrieveChanges(Run build, GitClient git, TaskListener listener)
		...
		if (repos.isEmpty())
			return; // defensive check even though this is an invalid
					// configuration
		...
		if (git.hasGitRepo()) {
			...
			if (repos.size() == 1)
				...
				log.println(MessageFormat.format(
						"Fetching changes from {0} remote Git repositories",
						repos.size()));
		} else {
			...
			try {
				CloneCommand cmd = git.clone_()
						.url(rc.getURIs().get(0).toPrivateString())
		...
		for (RemoteConfig remoteRepository : repos) {
			try {
				 ...
				 */
				String message = "Error fetching remote repo '"
	...
	public void checkout(Run<?, ?> build, Launcher launcher,
		...
		BuildData previousBuildData = getBuildData(build.getPreviousBuild()); // read
																				// only
		BuildData buildData = copyBuildData(build.getPreviousBuild());
		build.addAction(buildData);
		if (VERBOSE && buildData.lastBuild != null) {
			listener.getLogger().println(
					"Last Built Revision: " + buildData.lastBuild.revision);
		}
		...
		Build revToBuild = determineRevisionToBuild(build, buildData,
				environment, git, listener);

		environment.put(GIT_COMMIT, revToBuild.revision.getSha1String());
		Branch branch = Iterables.getFirst(revToBuild.revision.getBranches(),
				null);
		...
		try {
			...
			throw new IOException("Could not checkout "
					+ revToBuild.revision.getSha1String(), e);
		}

		build.addAction(new GitTagAction(build, workspace, buildData));

		if (changelogFile != null) {
			computeChangeLog(git, revToBuild.revision, listener,
					previousBuildData, new FilePath(changelogFile),
					new BuildChooserContextImpl(build.getParent(), build,
							environment));
		}
	...
	/**
	 * Build up change log from all the branches that we've merged into
	 * {@code revToBuild}.
	 * 
	 * <p>
	 * Intuitively, a changelog is a list of commits that's added since the
	 * "previous build" to the current build. However, because of the multiple
	 * branch support in Git, this notion is ambiguous. For example, consider
	 * the following commit graph where M1...M4 belongs to branch M, B1..B2
	 * belongs to branch B, and so on:
	 * 
	 * <pre>
	 *    M1 -> M2 -> M3 -> M4
	 *  /   \     \     \
	 ...
	 * 
	 * <p>
	 * If Jenkin built B1, C1, B2, C3 in that order, then one'd prefer that the
	 * changelog of B2 only shows just B1..B2, not C1..B2. To do this, we
	 * attribute every build to specific branches, and when we say
	 * "since the previous build", what we really mean is
	 * "since the last build that built the same branch".
	 * 
	 * <p>
	 * TODO: if a branch merge is configured, then the first build will end up
	 * listing all the changes in the upstream branch, which may be too many. To
	 * deal with this nicely, BuildData needs to remember when we started
	 * merging this branch so that we can properly detect if the current build
	 * is the first build that's merging a new branch.
	 * 
	 * Another possibly sensible option is to always exclude all the commits
	 * that are happening in the remote branch. Picture yourself developing a
	 * feature branch that closely tracks a busy mainline, then you might not
	 * really care the changes going on in the main line. In this way, the
	 * changelog only lists your changes, so "notify those who break the build"
	 * will not spam upstream developers, too.
	 * 
	 * @param git
	 *            Used for invoking Git
	 * @param revToBuild
	 *            Points to the revision we'll be building. This includes all
	 *            the branches we've merged.
	 * @param listener
	 *            Used for writing to build console
	 * @param previousBuildData
	 *            Information that captures what we did during the last build.
	 *            We need this for changelog, or else we won't know where to
	 *            stop.
	 */
	private void computeChangeLog(GitClient git, Revision revToBuild,
			TaskListener listener, BuildData previousBuildData,
			FilePath changelogFile, BuildChooserContext context)
			throws IOException, InterruptedException {
		...
		try {
			...
			ChangelogToBranch changelogToBranch = getExtensions().get(
					ChangelogToBranch.class);
			if (changelogToBranch != null) {
				...
				for (Branch b : revToBuild.getBranches()) {
					Build lastRevWas = getBuildChooser().prevBuildForChangelog(
							b.getName(), previousBuildData, git, context);
					if (lastRevWas != null && lastRevWas.revision != null
							&& git.isCommitInRepo(lastRevWas.getSHA1())) {
			...
			if (!exclusion) {
				// this is the first time we are building this branch, so
				// there's no base line to compare against.
				// if we force the changelog, it'll contain all the changes in
				// the repo, which is not what we want.
				listener.getLogger().println(
	...
	public void buildEnvVars(AbstractBuild<?, ?> build,
		...
		if (rev != null) {
			...
			if (branch != null) {
				...
				String prevSuccessfulCommit = getLastSuccessfulBuiltCommitOfBranch(
						build, branch);
				if (prevSuccessfulCommit != null) {
					env.put(GIT_PREVIOUS_SUCCESSFUL_COMMIT,
							prevSuccessfulCommit);
				}
			}
	...
	private String getLastBuiltCommitOfBranch(AbstractBuild<?, ?> build,
			Branch branch) {
		String prevCommit = null;
		if (build.getPreviousBuiltBuild() != null) {
			final Build lastBuildOfBranch = fixNull(
					getBuildData(build.getPreviousBuiltBuild()))
					.getLastBuildOfBranch(branch.getName());
	...
	private String getLastSuccessfulBuiltCommitOfBranch(
			AbstractBuild<?, ?> build, Branch branch) {
		String prevCommit = null;
		if (build.getPreviousSuccessfulBuild() != null) {
			final Build lastSuccessfulBuildOfBranch = fixNull(
					getBuildData(build.getPreviousSuccessfulBuild()))
					.getLastBuildOfBranch(branch.getName());
			if (lastSuccessfulBuildOfBranch != null) {
				Revision previousRev = lastSuccessfulBuildOfBranch
						.getRevision();
				if (previousRev != null) {
					prevCommit = previousRev.getSha1String();
				}
	...
	public ChangeLogParser createChangeLogParser() {
		return new GitChangeLogParser(getExtensions().get(
				AuthorInChangelog.class) != null);
	}
	...
	public static final class DescriptorImpl extends SCMDescriptor<GitSCM> {
		...
		public void setCreateAccountBasedOnEmail(
				boolean createAccountBasedOnEmail) {
			this.createAccountBasedOnEmail = createAccountBasedOnEmail;
		}

		/**
		 * Old configuration of git executable - exposed so that we can migrate
		 * this setting to GitTool without deprecation warnings.
		 */
		...
		/**
		 * Determine the browser from the scmData contained in the
		 * {@link StaplerRequest}.
		 * 
		 * @param scmData
		 * @return browser based on request scmData
		 */
		private GitRepositoryBrowser getBrowserFromRequest(
				final StaplerRequest req, final JSONObject scmData) {
			if (scmData.containsKey("browser")) {
				return req.bindJSON(GitRepositoryBrowser.class,
						scmData.getJSONObject("browser"));
			} else {
				return null;
			}
		}

		public static List<RemoteConfig> createRepositoryConfigurations(
				String[] urls, String[] repoNames, String[] refs)
				throws IOException {
			...
			for (int i = 0; i < names.length; i++) {
				...
				repoConfig.setStringList(
						...
						name,
						"fetch",
						new ArrayList<String>(Arrays.asList(refs[i]
								.split("\\s+"))));
			}

			try {
				remoteRepositories = RemoteConfig
						.getAllRemoteConfigs(repoConfig);
			} catch (Exception e) {
		...
		public static PreBuildMergeOptions createMergeOptions(
				...
				List<RemoteConfig> remoteRepositories) throws FormException {
			PreBuildMergeOptions mergeOptions = new PreBuildMergeOptions();
			if (mergeOptionsBean != null) {
				RemoteConfig mergeRemote = null;
				String mergeRemoteName = mergeOptionsBean.getMergeRemote()
						.trim();
				...
				if (mergeRemote == null) {
					throw new FormException(
							"No remote repository configured with name '"
									+ mergeRemoteName + "'", "git.mergeRemote");
				}
				mergeOptions.setMergeRemote(mergeRemote);
				mergeOptions.setMergeTarget(mergeOptionsBean.getMergeTarget());
				mergeOptions.setMergeStrategy(mergeOptionsBean
						.getMergeStrategy());
			}
	...
	public PreBuildMergeOptions getMergeOptions() throws FormException {
		return DescriptorImpl.createMergeOptions(getUserMergeOptions(),
				remoteRepositories);
	}
	 ...
	 * completed. BuildData may not be recorded if an exception occurs in the
	 * plugin logic.
	 * 
	 * @param build
	 ...
	 */
	public @CheckForNull
	BuildData getBuildData(Run build) {
	...
	/**
	 * Given the workspace, gets the working directory, which will be the
	 * workspace if no relative target dir is specified. Otherwise, it'll be
	 * "workspace/relativeTargetDir".
	 * 
	 * @param workspace
	 * @return working directory or null if workspace is null
	 */
	protected FilePath workingDirectory(Job<?, ?> context, FilePath workspace,
		...
		for (GitSCMExtension ext : extensions) {
			FilePath r = ext.getWorkingDirectory(this, context, workspace,
					environment, listener);
			if (r != null)
				return r;
		}
	...
	/**
	 * Given a Revision "r", check whether the list of revisions
	 * "COMMITS_WE_HAVE_BUILT..r" are to be entirely excluded given the
	 * exclusion rules
	 * 
	 * @param git
	 *            GitClient object
	 ...
	 */
	private boolean isRevExcluded(GitClient git, Revision r,
			TaskListener listener, BuildData buildData) throws IOException,
			InterruptedException {
		try {
			List<String> revShow;
			if (buildData != null && buildData.lastBuild != null) {
				revShow = git.showRevision(
						buildData.lastBuild.revision.getSha1(), r.getSha1());
			} else {
			...
			for (String line : revShow) {
				if (line.startsWith("commit ") && idx != 0) {
					GitChangeSet change = new GitChangeSet(revShow.subList(
							start, idx), getExtensions().get(
							AuthorInChangelog.class) != null);

					Boolean excludeThisCommit = null;
					for (GitSCMExtension ext : extensions) {
						excludeThisCommit = ext.isRevExcluded(this, git,
								change, listener, buildData);
						if (excludeThisCommit != null)
							break;
					}
					if (excludeThisCommit == null || !excludeThisCommit)
						return false; // this sequence of commits have one
										// commit that we want to build
					start = idx;
				}
			...
			e.printStackTrace(listener
					.error("Failed to determine if we want to exclude "
							+ r.getSha1String()));
			return false; // for historical reason this is not considered a
							// fatal error.
		}
	}
	...
	public static void onLoaded() {
		DescriptorImpl desc = Jenkins.getInstance().getDescriptorByType(
		...
		if (desc.getOldGitExe() != null) {
			...
			System.err
					.println("[WARNING] you're using deprecated gitexe attribute to configure git plugin. Use Git installations");
		}
	}
	...
	public static void configureXtream() {
		...
		Items.XSTREAM
				.registerConverter(new RemoteConfigConverter(Items.XSTREAM));
	...
	/**
	 * Set to true to enable more logging to build's {@link TaskListener}. Used
	 * by various classes in this package.
	 */
	public static boolean VERBOSE = Boolean.getBoolean(GitSCM.class.getName()
			+ ".verbose");

	/**
	 * To avoid pointlessly large changelog, we'll limit the number of changes
	 * up to this.
	 */
	public static final int MAX_CHANGELOG = Integer.getInteger(
			GitSCM.class.getName() + ".maxChangelog", 1024);
}
