...
public class GitBlitRepositoryBrowserTest {
    ...
    public void testGetDiffLink() throws Exception {
        ...
        for (GitChangeSet.Path path : sample.changeSet.getPaths()) {
            ...
            assertTrue(
                    "Unexpected edit type " + editType.getName(),
                    editType == EditType.ADD || editType == EditType.EDIT || editType == EditType.DELETE);
            URL diffLink = gitblit.getDiffLink(path);
            URL expectedDiffLink =
                    new URL(repoUrl + "blobdiff?r=" + projectName + "&h=" + sample.id + "&hb=" + sample.parent);
    ...
    public void testGetFileLink() throws Exception {
        ...
        for (GitChangeSet.Path path : sample.changeSet.getPaths()) {
            ...
            if (editType == EditType.ADD || editType == EditType.EDIT) {
                expectedFileLink = new URL(repoUrl + "blob?r=" + projectName + "&h=" + sample.id + "&f="
