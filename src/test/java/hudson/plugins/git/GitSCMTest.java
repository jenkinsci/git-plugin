...
//        FreeStyleProject project = setupProject("master", true, false);
    ...
    public void testFirstBuildHasChangelogWithTwoBranches() throws Exception {
        ...
        sampleRepo.write("file", "v3");
        sampleRepo.git("commit", "--all", "--message=third");
        ObjectId knownSha1 = ObjectId.fromString(sampleRepo.head());
        ...
        d.addRemoteUrl(((GitSCM) project.getScm()).getUserRemoteConfigs().get(0).getUrl());
        d.getBuildsByBranchName().put("master", new hudson.plugins.git.util.Build(new Revision(knownSha1), 0, Result.SUCCESS));
        build.addAction(d);
    ...
    @Issue("JENKINS-14138")
    @Test
    public void testFirstBuildHasChangelogWithDetachedHeadAndTargetBranchAhead() throws Exception {
        sampleRepo.init();
        sampleRepo.write("file", "v1");
        sampleRepo.git("commit", "--all", "--message=first");
        sampleRepo.write("file", "v2");
        ...
        sampleRepo.git("commit", "--all", "--message=third");
        sampleRepo.git("checkout", "develop");
