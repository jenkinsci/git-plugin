...
public class MultibranchProjectTraitsTest {
    ...
    */
    ...
    public void basicSharedLibrarySymbolsTest() throws Exception {
        story.then(r -> {
            GlobalLibraries.get()
                    .setLibraries(Collections.singletonList( // TODO: Something more interesting with traits, maybe
                            // gitBranchDiscovery
                            new LibraryConfiguration(
                                    "thelibrary",
                                    new SCMSourceRetriever(new GitSCMSource(sharedLibrarySampleRepo.toString())))));
            ;
    ...
    */
    ...
    public void basicSharedLibraryClassTest() throws Exception {
        story.then(r -> {
            GlobalLibraries.get()
                    .setLibraries(Collections.singletonList(new LibraryConfiguration(
                            "thelibrary",
                            new SCMSourceRetriever(new GitSCMSource(sharedLibrarySampleRepo.toString())))));
            WorkflowJob job = story.j.jenkins.createProject(WorkflowJob.class, "test-pipeline-job");
            job.setDefinition(new CpsFlowDefinition(
